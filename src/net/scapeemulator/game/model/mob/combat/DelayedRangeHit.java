...
public class DelayedRangeHit extends Task {
	...
	public void execute() {
		...
		if (dropId > 0 && dropAmt > 0) {
			GroundItemList groundItemList = source instanceof Player ? ((Player) source).getGroundItems() : World.getWorld().getGroundItems();
			groundItemList.add(dropId, dropAmt, target.getPosition());
		}
		stop();
	}
