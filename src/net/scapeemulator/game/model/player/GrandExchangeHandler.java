...
public class GrandExchangeHandler extends IntegerScriptInputListener {
	...
	public void handleOfferInventoryClick(int childId, int dynamicId) {
		...
		if (item.getDefinition().isNoted()) {
			activeItemId = item.getDefinition().getUnnotedId();
		} else {
	...
	public void submitOffer() {
		...
		if (type == OfferType.BUY) {
			...
			if(removed == null || !removed.equals(toRemove)) {
				...
				toRemove = new Item(toRemove.getDefinition().getNotedId(), removed != null ? toRemove.getAmount() - removed.getAmount() : toRemove.getAmount());
	...
	public void collect(boolean itemSlot, boolean noted) {
		...
		if (itemSlot) {
			if (noted) {
				Item remaining = player.getInventory().add(new Item(ItemDefinitions.forId(offer.getItemId()).getNotedId(), offer.getUnclaimedItems()));
				if (remaining != null) {
