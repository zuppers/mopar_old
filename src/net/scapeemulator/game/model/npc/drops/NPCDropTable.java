...
/**
 * Represents a drop table collection for a NPC.
 * 
 * @author David Insley
 */
public class NPCDropTable {
    ...
    public static final NPCDropTable[] DROP_TABLES = new NPCDropTable[NPCDefinitions.count()];

    private Map<TableType, DropTable> tables = new HashMap<>();

    /**
     * Adds a drop table with the default table chance. Overwrites any existing table for that type.
     * 
     * @param type the TableType to add
     */
    public void addTable(TableType type) {
        tables.put(type, new DropTable(type.getDefaultChance()));
    }

    /**
     * Adds a drop table with the specified table chance. Overwrites any existing table for that
     * type. The chance should be a value between 0 (inclusive) and 1 (exclusive), with lower values
     * being more rare.
     * 
     * @param type the TableType to add
    ...
    public void addTable(TableType type, double chance) {
        ...
        tables.put(type, new DropTable(chance));
    }

    /**
     * Adds a drop to the specified table with the special string amount argument. Amount argument
     * should be in the format: "<tt>amt1,amt2,amt3min-amt3max,amt4</tt>". More specifically,
     * amounts should be separated by commas and ranges specified by dashes.
     * 
    ...
    public List<Item> getRandomDrops() {
        ...
        DropTable activeTable = tables.get(TableType.ALWAYS);
        if (activeTable != null) {
        ...
        // See if they should get an extra drop from the extras table
